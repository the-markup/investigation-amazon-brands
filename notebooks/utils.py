...
def calculate_table(df, amazon_asin):
    ...
    df_no_prime_ =  df_ns[df_ns.is_shipped_by_amazon_TRUE != True]
    ...
    res = pd.DataFrame([
        ...
        {
            'Category': 'Third-party shipped',
            'Perc Products': len(df_no_prime_) / n_products * 100,
            'Perc #1 spot': df_no_prime_[df_no_prime_.product_order == 1].search_term.nunique() / n_samples* 100,
            'Perc first row': df_no_prime_[df_no_prime_.product_order <= 3].search_term.nunique() / n_samples* 100,
        },
    ])
